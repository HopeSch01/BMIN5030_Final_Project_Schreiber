...
mem_results <- memmodel(
  ...
  i.type.threshold = 1,      #option 1 = arithmetic mean and SDs
  i.tails.threshold = 0.95,  #uses 95th percentile of historical epidemic activity to set the threshold
  i.type.intensity = 6,      #option 6 = geometric mean and SDs
  i.level.intensity = c(0.40, 0.90, 0.975), 
  #low intensity = below 40th percentile 
  ...
  i.param = 2.8,             #controls shape or spread of epidemic curve
  i.n.max = -1,              #consider all weeks  
  i.type.boot = "norm",      #bootstrap distribution used to estimate CIs for thresholds and intesnities = normal
...
mem_thresholds <- data.frame(
  ...
  value = c(
    mem_results$epidemic.thresholds[1],  # Pre-epidemic
    mem_results$epidemic.thresholds[2],  # Post-epidemic
...
Vega et al. (2013) recommends using the geometric mean to calculate thresholds because epidemic peaks are inherently skewed. Since here we are working with state level data, our percent ILI values are smaller. The geometric mean is sensitive to values near zero - which is why in this project the arithmetic mean was used instead.

The mem model above provides a pre- and post- epidemic threshold, as well as three epidemic thresholds. These percentages correspond to the percent of medical visits related to ILI diagnosis in any given week.
