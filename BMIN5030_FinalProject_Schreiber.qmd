...
#Loading Regional Hospitalization Data 
FluSurv_Hospitalizations <- read_csv("C:/Users/hopeh/Downloads/Fall 2025/EPID 6000 Data Science for Biomedical Informatics/Final Project/BMIN5030_Final_Project_Schreiber/FluSurveillance_Hospitalizations.csv")

#renaming for clarity 
names(FluSurv_Hospitalizations)[names(FluSurv_Hospitalizations) == "YEAR...3"] <- "flu.season"

names(FluSurv_Hospitalizations)[names(FluSurv_Hospitalizations) == "YEAR...4"] <- "YEAR"

#Cleaning Data 
FluSurv_Hospitalizations_clean <- FluSurv_Hospitalizations |> 
  filter(!is.na(flu.season)) |>
  mutate(
    CUMULATIVE_RATE = as.numeric(`CUMULATIVE RATE`),
    WEEKLY_RATE = as.numeric(`WEEKLY RATE`),
    AGE_ADJ_CUM_RATE = as.numeric(`AGE ADJUSTED CUMULATIVE RATE`),
    AGE_ADJ_WEEKLY_RATE = as.numeric(`AGE ADJUSTED WEEKLY RATE`),
    #Clean up other columns
    flu.season = as.factor(flu.season),
    YEAR = as.numeric(YEAR),
    WEEK = as.factor(WEEK),
    AGE_CATEGORY = `AGE CATEGORY`,
    SEX_CATEGORY = `SEX CATEGORY`,
    RACE_CATEGORY = `RACE CATEGORY`,
    VIRUS_TYPE = `VIRUS TYPE CATEGORY`
  ) |>
  #Remove rows where all rate columns are NA
  filter(!is.na(CUMULATIVE_RATE) | !is.na(WEEKLY_RATE))

#re-ordering CDC weeks for plot clarity 
FluSurv_Hospitalizations_clean$WEEK <- factor(FluSurv_Hospitalizations_clean$WEEK, levels = c(40:53, 1:20))

#Filtering patterns for variables 

#Age
age_data <- ILINet_Hospitalizations_clean |>
  filter(!is.na(AGE_CATEGORY)) |>
  filter(!AGE_CATEGORY %in% c("0-< 1yr", "0-4 yr", "< 18", ">= 18", ">= 65 yr", ">= 75", "Overall"))

#Sex
sex_data <- ILINet_Hospitalizations_clean |>
  filter(!is.na(SEX_CATEGORY)) |>
  filter(!is.na(SEX_CATEGORY) & SEX_CATEGORY != "Overall")

#Race
race_data <- ILINet_Hospitalizations_clean |>
  filter(!is.na(RACE_CATEGORY)) |>
  filter(!is.na(RACE_CATEGORY) & RACE_CATEGORY != "Overall")

#Influenza Type 
virus_data <- ILINet_Hospitalizations_clean |>
  filter(!is.na(VIRUS_TYPE) & 
         VIRUS_TYPE != "Overall" & 
         VIRUS_TYPE != "A(H1N1)pdm09" & 
         VIRUS_TYPE != "A(H3N2)")
```

This data consists of regional influenza hospitalization rates (per 100,000 population) for the FluSurv-NET Network. This network is part of the Influenza division of the CDC.
...
##FluSurv-NET Data
head(FluSurv_Hospitalizations_clean)
dim(FluSurv_Hospitalizations_clean)
glimpse(FluSurv_Hospitalizations_clean)
...
The FluSurv_Hospitalizations\>clean dataframe has 21 variables and 23,683 rows of data.
...
### Exploring Regional Influenza Hospitalization Trends

```{r}

#Summary statistics by flu season 

year_summary <- ILINet_Hospitalizations_clean |>
  group_by(flu.season) |>
  summarise(
    n_obs = n(),
    mean_rate = mean(CUMULATIVE_RATE, na.rm = TRUE),
    median_rate = median(CUMULATIVE_RATE, na.rm = TRUE),
    sd_rate = sd(CUMULATIVE_RATE, na.rm = TRUE),
    min_rate = min(CUMULATIVE_RATE, na.rm = TRUE),
    max_rate = max(CUMULATIVE_RATE, na.rm = TRUE),
    q25_rate = quantile(CUMULATIVE_RATE, 0.25, na.rm = TRUE),
    q75_rate = quantile(CUMULATIVE_RATE, 0.75, na.rm = TRUE),
    na_count = sum(is.na(CUMULATIVE_RATE))
  ) |>
  arrange(flu.season)
...
age_summary <- ILINet_Hospitalizations_clean |>
...
#Rates over time by flu season 

#calculating weekly rates by flu season 
weekly_rates <- ILINet_Hospitalizations_clean |>
  group_by(flu.season, WEEK) |>
  summarise(
    mean_rate = mean(CUMULATIVE_RATE, na.rm = TRUE),
    median_rate = median(CUMULATIVE_RATE, na.rm = TRUE),
    .groups = 'drop'
  ) |>
  filter(!is.na(mean_rate) & !is.infinite(mean_rate))

#Time series of hospitalization rates by flu season
hosp_flu_season <- ggplot(weekly_rates, aes(x = WEEK, y = mean_rate, color = flu.season, group = flu.season)) +
  geom_line(linewidth = 1) +
  geom_point(size = 1.5, alpha = 0.6) +
  labs(
    title = "Influenza Hospitalization Rates Over Time by Flu Season",
    subtitle = "Mean cumulative rate by week",
    x = "Week",
    y = "Mean Cumulative Rate (per 100,000)",
    color = "Flu Season"
  ) +
...
#Rates over time by age category 

#Calculate rates by age category and week (across all seasons)
age_weekly_rates <- age_data |>
  group_by(AGE_CATEGORY, WEEK) |>
  summarise(
    mean_rate = mean(CUMULATIVE_RATE, na.rm = TRUE),
    .groups = 'drop'
  ) |>
...
hosp_age <- ggplot(age_weekly_rates, aes(x = WEEK, y = mean_rate, color = AGE_CATEGORY, group = AGE_CATEGORY)) +
  geom_line(linewidth = 1.2) +
  labs(
    title = "Influenza Hospitalization Rates by Age Category",
    subtitle = "Mean cumulative rate across all flu seasons",
    x = "Week",
    y = "Mean Cumulative Rate (per 100,000)",
    color = "Age Category"
  ) +
...
#Hospitalization Burden by Virus Type and Season 

#Calculate total/mean rates (not just counts of observations)
virus_season_burden <- virus_data |>
  group_by(flu.season, VIRUS_TYPE) |>
  summarise(
    total_rate = sum(CUMULATIVE_RATE, na.rm = TRUE),
    mean_rate = mean(CUMULATIVE_RATE, na.rm = TRUE),
    n_obs = n(),
    .groups = 'drop'
  ) |>
  group_by(flu.season) |>
  mutate(
    prop_by_rate = total_rate / sum(total_rate),
    prop_by_mean = mean_rate / sum(mean_rate)
  )

hosp_virus_type <- ggplot(virus_season_burden, aes(x = flu.season, y = prop_by_mean, fill = VIRUS_TYPE)) +
  geom_col(position = "stack") +
  labs(
    title = "Virus Type Disease Burden by Flu Season",
    subtitle = "Proportion based on mean hospitalization rates",
    x = "Flu Season",
...
#Mean hospitalization rates by sex
...
hosp_sex <- ggplot(sex_summary, aes(x = SEX_CATEGORY, y = mean_rate, fill = SEX_CATEGORY)) +
  geom_col(alpha = 0.8) +
  geom_errorbar(aes(ymin = pmax(0, mean_rate - sd_rate), 
                    ymax = mean_rate + sd_rate),
                width = 0.3, alpha = 0.6) +
  labs(
    title = "Mean Influenza Hospitalization Rates by Sex",
...
#Mean hospitalization rates by race 
...
hosp_race <- ggplot(race_summary, aes(x = reorder(RACE_CATEGORY, mean_rate), 
                                      y = mean_rate, fill = RACE_CATEGORY)) +
  geom_col(alpha = 0.8) +
  geom_errorbar(aes(ymin = pmax(0, mean_rate - sd_rate), 
                    ymax = mean_rate + sd_rate),
                width = 0.3, alpha = 0.6) +
  coord_flip() +
  labs(
    title = "Mean Influenza Hospitalization Rates by Race/Ethnicity",
    subtitle = "Error bars represent ±1 standard deviation",
    x = "Race/Ethnicity",
    y = "Mean Cumulative Rate (per 100,000)"
  ) +
...
#### Statistically Analyzing Influenza Hospitalization Data

```{r}

##Part 1. Chi-Square Tests##

#Age x High/Low Hospitalization Rate
age_data <- age_data |>
  mutate(
    high_rate = ifelse(CUMULATIVE_RATE > median(CUMULATIVE_RATE, na.rm = TRUE), 
                       "High", "Low"),
    high_rate = factor(high_rate, levels = c("Low", "High"))
  )

chi_age <- chisq.test(table(age_data$AGE_CATEGORY, age_data$high_rate))
print(chi_age)


#Sex x High/Low Hospitalization Rate
sex_data <- sex_data |>
  mutate(
    high_rate = ifelse(CUMULATIVE_RATE > median(CUMULATIVE_RATE, na.rm = TRUE), 
                       "High", "Low"),
...
chi_sex <- chisq.test(table(sex_data$SEX_CATEGORY, sex_data$high_rate))
print(chi_sex)


#Race x High/Low Hospitalization Rate
race_data <- race_data |>
  mutate(
    high_rate = ifelse(CUMULATIVE_RATE > median(CUMULATIVE_RATE, na.rm = TRUE), 
                       "High", "Low"),
...
chi_race <- chisq.test(table(race_data$RACE_CATEGORY, race_data$high_rate))
print(chi_race)


#Virus Type x High/Low Hospitalization Rate
...
chi_virus <- chisq.test(table(virus_data$VIRUS_TYPE, virus_data$high_rate))
print(chi_virus)
...
#Age model
lm_age <- lm(CUMULATIVE_RATE ~ AGE_CATEGORY, data = age_data)
summary(lm_age)

#Sex model
lm_sex <- lm(CUMULATIVE_RATE ~ SEX_CATEGORY, data = sex_data)
summary(lm_sex)

#Race model
lm_race <- lm(CUMULATIVE_RATE ~ RACE_CATEGORY, data = race_data)
summary(lm_race)

#Virus model
lm_virus <- lm(CUMULATIVE_RATE ~ VIRUS_TYPE, data = virus_data)
summary(lm_virus)
...
##Part 3. Simple Logistic Regression

# Age model
logit_age <- glm(high_rate ~ AGE_CATEGORY, data = age_data, family = binomial)
summary(logit_age)

# Get odds ratios
exp(coef(logit_age))
exp(confint(logit_age))

# Sex model
logit_sex <- glm(high_rate ~ SEX_CATEGORY, data = sex_data, family = binomial)
summary(logit_sex)

# Race model
logit_race <- glm(high_rate ~ RACE_CATEGORY, data = race_data, family = binomial)
summary(logit_race)

# Virus model
logit_virus <- glm(high_rate ~ VIRUS_TYPE, data = virus_data, family = binomial)
summary(logit_virus)
...
A few key differences from the WHO method including using the Maximum Accumulated Percentage (MAP) curve instead of arbitrary percentiles, as well as providing more intensity levels \[11\].
...
-   connect different thresholds to specific interventions

-   how do you decide what inputs to put into the model?

    -   What are the optimal setting for this model?

    -   How much thresholds might change based on changes in inputs?

-   at what point have historical interventions been triggered?

    -   try to tune the model to these interventions

-   could create a vector of different inputs - loop that over

    -   graph inputs versus thresholds
